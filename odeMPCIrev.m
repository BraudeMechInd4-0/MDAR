function [tout,xout,errorOutput,model] = odeMPCIrev(ODEFUN, TSPAN, Xf,options)
...
%    t_secs(end+1) = tf;
%end
section_num = ceil(tf/Sec);%length(t_secs)-1;
sec_count = section_num;
tstart = floor(tf/Sec) * Sec;%t_secs(sec_count); 
% temp_t = 0;
% for i = 1:section_num-1
%     temp_t = temp_t + Sec;
% end
% tstart = temp_t;  
tend = tf;%t_secs(sec_count+1);
if length(TSPAN) == 2
    output_length = N*ceil(TSPAN(end)/Sec)+1;
else
    output_length = length(TSPAN);
end
%firstpos = output_length-N+1;
lastpos = output_length;
vector_length = length(Xf);
xout = NaN(output_length,vector_length);
xout(end,:) = Xf;
tout = NaN(output_length,1);

if outputmodel
    %model_length = section_num;
    %model_counter = section_num;
    model.ts = zeros(section_num+1,1);
    model.ts(end) = tf;
    model.bi = NaN(section_num,N+1,vector_length);
else
    model = [];
end
...
if ~isfield(options,'xinit')
    ...
    xinit = repmat(Xf,[length(tau),1]); %innitial guess
else
    xinit = options.xinit;
end

Xf = [Xf;zeros(N,length(Xf))];
if Sec < TSPAN(end)
    lastrunflag = false;
else
    lastrunflag = true;
...
Tm1 = ones(1,N+1); %acos(1) = 0 then cos(0) = 1 for all k
L = [Tm1;zeros(N,N+1)];
s_ = 1./(4:2:2*N);
S_3 = zeros(N+1);
...
while true
    ...
    if length(TSPAN) ~= 2
        ...
        if isempty(relevantTSPAN)
            ...
            tstart = t_secs(sec_count);
            tend = t_secs(sec_count+1);
            ...
            % end
            Xf_ = xnew(1,:);
            xinit = repmat(Xf_,[length(tau),1]); %innitial guess
            Xf = [Xf_;zeros(N,length(Xf_))];
        ...
        firstpos = lastpos - length (relevantTSPAN);
        tout(firstpos+1:lastpos) = relevantTSPAN;
        T_ = cos((0:N)'.*acos(tau_))'; %length(tau)xN+1
        xout_ = T_ * bi;
        xout(firstpos+1:lastpos,:) = xout_;
    else
        %lastpos = firstpos+N;
        firstpos = lastpos-N;
    ...
    if outputmodel
        model.bi(sec_count,:,:) = bi;
        model.ts(sec_count) = tstart;
    end
    % if lastrunflag
    %     return
    % end
    ...
    %Get the next (previous in time) segment
    tend = tstart;
    tstart = tend-Sec;
    if sec_count < 1 || tend <= 0
