function [dr] = orbit_eq_J6_drag_SRP_moon(t,r,muE,CD,Adrag,m,Re,J)%,pp_sun,pp_moon)
%ORBIT_EQ_J6_DRAG_SRP_MOON  EOM: J2-J6 zonal + drag + SRP + lunar third body.
%   dr = ORBIT_EQ_J6_DRAG_SRP_MOON(t, r, muE, CD, Adrag, m, Re, J) returns the state
%   derivative for a spacecraft under Earth gravity with zonal harmonics J2 through
%   J6, atmospheric drag, solar radiation pressure, and lunar third-body attraction.
%   Sun and Moon positions are obtained from sun_v/moon_v at Julian date jdepoch + t.
%
%   Inputs:
%     t     - time(s) since epoch [s]
%     r     - state [rx ry rz vx vy vz] (6xN column-wise, or Nx6 row-wise)
%     muE   - Earth gravitational parameter [km^3/s^2]
%     CD    - drag coefficient
%     Adrag - cross-sectional area [km^2] (also used as the SRP area)
...
%
%   Hardcoded constants (below): jdepoch (reference Julian date), rhoSRP (SRP
%   pressure), CR (reflectivity coefficient), muM (lunar gravitational parameter).
